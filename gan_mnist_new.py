...
# data = data.to(device)
data = data / np.linalg.norm(data, axis=0) * np.sqrt(n)
ipca.fit(data)
...
print(np.min(loadings), np.max(loadings))

# loadings = (loadings + np.min(loadings)) / (np.max(loadings) - np.min(loadings))
# loadings = (loadings + 1.0) / 2.0
...
sigma = torch.diag(torch.ones((d))) * 20
eta = 1
...
for _ in range(num_epochs):
    ...
    for _, batch in enumerate(training_dataloader):
        ...
        for idx, image in enumerate(pbar):
            ...
            image = (image - torch.mean(image)) / torch.std(image, dim=0) # Centering data
            ...
            if idx % 10 == 0:# and not updated_loss:
                ...
                grassmann_distances.append(grassmann_distance(V.cpu().numpy(), loadings))
...
# U, S, Vh = torch.svd(torch.from_numpy(loadings).to(device))
# U = torch.from_numpy(loadings).to(device)
if dataset == "cifar10":
...
# top_16 = (top_16 * 255.0).astype(np.uint8)
plt.figure()
for i in range(d):
    plt.subplot(int(sqrt(d)), int(sqrt(d)), i+1)
...
plt.figure()
plt.plot(np.arange(0, len(grassmann_distances) * 10, 10), np.array(grassmann_distances))
...
plt.clf()
test_image = next(iter(training_dataloader))[0][0]
print(test_image.shape)
test_image = test_image.flatten().unsqueeze(1).to(device)

recon_image = U @ U.T @ test_image

recon_image = (recon_image.reshape(32, 32, -1) * 255.0).cpu().numpy()#.astype(np.uint8)

print(np.min(recon_image), np.max(recon_image))

print(recon_image.shape)

plt.imshow(recon_image.astype(np.uint8))
plt.savefig("test_recon_img.png")
